...
class CryoAsicAnalysis:
	...
	def calculate_psds_allsamples(self):
		chs = self.df.iloc[0]['Channels']
		nevents = len(self.df.index)
		looper = tqdm(chs, desc="Calculating PSD on channel with all points.")
		...
		for ch in looper:
			...
			for i in range(nevents):
				ev = self.df.iloc[i]	
				wave = ev["Data"][ch]	
				if (max(wave, key=abs) > 500) :
				#	print(f'skipped event {i} w/ Vmax > 50 mV.')
					continue

				wave = [_*self.config["mv_per_adc"]/1000. for _ in wave]

				cha_wv = cha_wv + wave

			freqs, pxx_tot = periodogram(cha_wv, self.sf*1e6)
